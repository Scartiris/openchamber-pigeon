  ...
  return (
    ...
    >
      ...
      <div className={cn('relative min-h-0 flex-1 overflow-hidden', isResizing && 'pointer-events-none')}>
        ...
        {activeTab?.mode !== 'chat' && !isFileTabActive && activeTab?.mode !== 'browser' && activeTab?.mode !== 'diff' && activeTab?.mode !== 'terminal' && activeTab?.mode !== 'walkthrough' && activeTab?.mode !== 'doc' && !(activeTab && isPluginContextPanelMode(activeTab.mode)) ? activeNonChatContent : null}
      </div>
      </div>
    </aside>
  );
